...
function onCreate()
setProperty('introSoundsSuffix','-pixel')
...
function onCreatePost()
    ...
    makeLuaSprite('icons1', nil, getProperty('iconP1.x') + 0, getProperty('iconP1.y') - 0)
    loadGraphic('icons1', 'icons/Bf_Blue_Icon', 150)
    setObjectCamera('icons1', 'hud')
    addLuaSprite('icons1')
    setProperty('icons1.flipX', true)

    makeLuaSprite('icons2', nil, getProperty('iconP2.x') + 0, getProperty('iconP2.y') - 0)
    loadGraphic('icons2', 'icons/Binky_icon', 150)
    setObjectCamera('icons2', 'hud')
    addLuaSprite('icons2')
 
    makeLuaSprite('icons2Cat', nil, getProperty('iconP1.x') + 0, getProperty('iconP1.y') - 0)
    loadGraphic('icons2Cat', 'icons/Arlene_Icon', 150)
    setObjectCamera('icons2Cat', 'hud')
    addLuaSprite('icons2Cat')
    setProperty('icons2Cat.visible', false)
    setProperty('icons2Cat.flipX', true)

makeLuaSprite('health', 'pixelUI/healthbar_pixel_blue', getProperty('healthBar.x'), getProperty('healthBar.y'))
...
setProperty('iconP1.y', getProperty('health.y') + -90)
setProperty('iconP2.y', getProperty('health.y') + -90)

--Score [Credits - uh.. nose quien lo hizo en verdad]
...
setProperty('icons2.x', getProperty('iconP1.x') + -130)
setProperty('icons2Cat.x', getProperty('iconP1.x') + 0)

setObjectOrder('health', getObjectOrder('healthBar') + 2)
setObjectOrder('filler', getObjectOrder('healthBar') + 4)
setObjectOrder('healthBar', getObjectOrder('healthBar') + 5)
setObjectOrder('icons1', getObjectOrder('icons1') + 7)
setObjectOrder('iconP1', getObjectOrder('iconP1') + 7)
setObjectOrder('icons2', getObjectOrder('icons2') + 7)
setObjectOrder('icons2Cat', getObjectOrder('icons2Cat') + 7)
setObjectOrder('iconP2', getObjectOrder('iconP2') + 7)
...
local h = getHealth() <= 0.35

    loadGraphic('icons1', 'icons/Bf_Blue_Icon', 150)
    addAnimation('icons1', 'icons/Bf_Blue_Icon', {h and 1 or 0, h and 0 or 1}, 0, true)
    loadGraphic('icons2Cat', 'icons/Arlene_Icon', 150)
    addAnimation('icons2Cat', 'icons/Arlene_Icon', {h and 1 or 0, h and 0 or 1}, 0, true)

local hh = (getProperty('healthBar.percent') > 80)

    loadGraphic('icons2', 'icons/Binky_icon', 150)
    addAnimation('icons2', 'icons/Binky_icon', {hh and 1 or 0, hh and 0 or 1}, 0, true)
end
...
setProperty('health.scale.y', '3.14')
setProperty('health.scale.x', '3.14')

setProperty('icons1.scale.y', '0.8')
setProperty('icons1.scale.x', '0.8')
setProperty('icons2.scale.y', '0.8')
setProperty('icons2.scale.x', '0.8')
setProperty('icons2Cat.scale.y', '0.8')
setProperty('icons2Cat.scale.x', '0.8')

setObjectCamera('comboGroup', 'camGame')
setProperty('showRating', false)
setProperty('showComboNum', false)

setProperty('health.y', getProperty('healthBar.y') + -15)
setProperty('health.x', getProperty('healthBar.x') + 161)
setProperty('filler.y', getProperty('healthBar.y') + 2)
setProperty('iconP2.x', getProperty('iconP2.x') + -65)
...
function onCountdownTick(counter)
	if counter == 1 then
loadGraphic('countdownReady','pixelUI/binky/Ready_blue')
runTimer('1',0.12)
 elseif counter == 2 then
  loadGraphic('countdownSet','pixelUI/binky/Set_blue')
  runTimer('2',0.12)
elseif counter == 3 then

 loadGraphic('countdownGo','pixelUI/binky/play_blue')
runTimer('3',0.12)
	end
...
function makeCombo(rate)
 ...
 makeLuaSprite('rating' .. time, 'pixelUI/binky/' .. rate, startX, startY)
...
makeLuaSprite('rating' .. time, 'pixelUI/' .. rate, startX, startY)
 ...
 if combo >= 10 then
  ...
  for i=1, long do
   ...
   makeLuaSprite('comboNum' .. i .. time, 'pixelUI/binky/num' .. string.sub(comboDisplay, i, i), startX - 49, startY + 139)
