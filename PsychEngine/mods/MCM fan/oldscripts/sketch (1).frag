...
float oga = flixel_texture2D( bitmap, uv).a;
    ...
    vec3 col;
	float x =  sin(0.3*iTime+uv.y*20000.0)*sin(0.7*iTime+uv.y*29.0)*sin(0.3+0.33*iTime+uv.y*31.0)*0.0007;
	...
	float scans = clamp( 10000000.35+0.35*sin(3.5*iTime+uv.y*openfl_TextureSize.y*1.5), 0.0, 1.0);
	...
	col*=0.5-0.65*vec3(clamp((mod(openfl_TextureCoordv.x, 2.0)-1.0)*2.0,0.0,1.0));
