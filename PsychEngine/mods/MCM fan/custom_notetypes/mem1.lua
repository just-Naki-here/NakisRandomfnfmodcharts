function onCreate()
	...
	makeAnimatedLuaSprite('1e','hitStatic',-250,-150)
	addAnimationByPrefix('1e','staticanim','staticANIMATION',24,true)
	objectPlayAnimation('1e','staticanim',false)
	setScrollFactor('1e', 0, 0);
...
function noteMiss(id, noteData, noteType, isSustainNote)
	if noteType == 'staticNote' then
		...
		runTimer('1e',0.4);
		addLuaSprite('1e', true);
		objectPlayAnimation('1e','staticanim',true)
	end
end

function onTimerCompleted(tag, loops, loopsLeft)
    if tag == 'glitchhide' then
		removeLuaSprite('1e', false);
    end
end