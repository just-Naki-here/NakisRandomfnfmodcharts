local realLength= 0
local micnextphase=123240
local micnextphasestep=1519
function onSongStart()
    realLength = getProperty('songLength')
    setProperty('songLength', 40220)
    if version < '0.7' then
...
function onStepHit()
    if curStep == 496 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = micnextphase},4,{ease = 'expoInOut'})
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
    end
    if curStep == micnextphasestep then
        if version >= '0.7' then
