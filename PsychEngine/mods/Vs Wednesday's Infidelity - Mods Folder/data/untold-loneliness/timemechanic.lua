...
local realLength= 0 
local rabbitScream= 128400
local rabbitRaaah= 90000

-- Function triggered when the song starts
function onSongStart()
    realLength = getProperty('songLength')  -- Store the original song length
    setProperty('songLength', 71440)  -- Temporarily set a new song length
...
function onStepHit()
    ...
    if curStep == 952 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = rabbitRaaah},10,{ease = 'expoInOut'})
        else
    ...
    if curStep == 1200 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = rabbitScream},0.62,{ease = 'expoInOut'})
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
    end
