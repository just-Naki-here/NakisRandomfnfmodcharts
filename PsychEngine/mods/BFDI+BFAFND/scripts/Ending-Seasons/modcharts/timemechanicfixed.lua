 ---@author Just_NAKI_here
 ---@About Time Mechanic Fixed (For Ending Seasons)
 ---@description Changes the song length at certain steps to create a time mechanic(fake time mechanic taken from Wednesday's Infedelity's Song Unknown Suffering) Also includes health resets at certain points
-- needed for autoloader
function onCreate()
    debugPrint("hi")
end
local realLength = 0
...
local songEnd = 348000 --end of song
...
function onStepHit()
    ...
    if curStep == 4352 then
        ...
        if version >= '0.7' then
            ...
            startTween('songLengthSwap','game',{songLength = pibbyWithGun},0.1,{ease = 'expoInOut'})
    ...
    if curStep == 4768 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = bfAgain},0.1,{ease = 'expoInOut'})
            setProperty('health', 1)
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
    ...
    if curStep == 4832 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = rubyLimps},0.1,{ease = 'expoInOut'})
            setProperty('health', 1)
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
    ...
    if curStep == 4896 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = bfAgainAgain},0.1,{ease = 'expoInOut'})
            setProperty('health', 1)
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
    ...
    if curStep == 4960 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = pibbyNoGun},0.1,{ease = 'expoInOut'})
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
    end
    if curStep == 5024 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = meIsHereFirey},0.1,{ease = 'expoInOut'})
            setProperty('health', 1)
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
    ...
    if curStep == 5504 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = needleAgain},0.1,{ease = 'expoInOut'})
            setProperty('health', 1)
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
    ...
    if curStep == 5536 then
        if version >= '0.7' then
            startTween('songLengthSwap','game',{songLength = songEnd},0.1,{ease = 'expoInOut'})
            setProperty('health', 1)
        else
            doTweenX('changeTimeUnknow','unknownTime',getProperty('songLength'),7,'expoInOut')
        end
