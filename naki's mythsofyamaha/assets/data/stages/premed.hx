...
function postUpdate() {
	...
	// trace(camGame.scroll.x + ', ' + camGame.scroll.y);
	...
	camGame.x = -(window.x - (Capabilities.screenResolutionX / 2 - window.width / 2));
	camGame.y = -(window.y - (Capabilities.screenResolutionY / 2 - window.height / 2));
}
