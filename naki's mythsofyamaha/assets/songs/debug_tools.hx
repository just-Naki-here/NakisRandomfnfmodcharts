...
function postCreate() {
    ...
    botplayTxt.y = 50; 
    botplayTxt.setFormat(Paths.font("IDK"), 32, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
...
function update(elapsed:Float) {
    ...
    if (FlxG.keys.justPressed.SEVEN) { //i got tired of exiting the song ok
...
function updateSpeed(speed:Float){
    ...
    for (strumLine in strumLines){
        if (strumLine.vocals != null) strumLine.vocals.pitch = speed;
    }
