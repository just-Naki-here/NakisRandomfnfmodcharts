...
var chomping:FlxSound = FlxG.sound.load(Paths.sound("game/genesis/chomping"));

public var skipTxt:FlxText;

function create() {
    skipTxt = new FlxText(400, 50, FlxG.width - 800, "Press SPACE Bar to skip cutscene", 32);
    skipTxt.setFormat(Paths.font("krabby.ttf"), 32, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
    skipTxt.visible = true;
    skipTxt.alpha = 0;
    skipTxt.borderSize = 1.25;
    skipTxt.camera = camHUD;

    if (!FlxG.save.data.genCrt){
        crtFx = null;
        crtFxCamGAME = null;

    }
    if (!FlxG.save.data.genBloom){
        bloom = null;
        bloomHUD = null;

    }
    if (!FlxG.save.data.genDither){
        dithering = null;
    }

    if (!FlxG.save.data.genGlitch){
        glitchBloom = null;
    }
...
function onSongStart() {
    ...
    skipOnce = true;
    FlxTween.tween(skipTxt, { alpha: 1 }, 3, { ease: FlxEase.quadOut }); 
    FlxTween.tween(skipTxt, { alpha: 0 }, 3, { ease: FlxEase.quadIn, startDelay: 8 }); 
    tvMood(1,9);
...
function generateHeadClickables(amount, time) {
    ...
    for (i in 0 ... amount) {
        ...
        add(newHead);
        var g:CustomShader = null;
        if (FlxG.save.data.genGlitch){
            g = new CustomShader('Glitch');
        ...
        FlxMouseEvent.add(newHead, (_) -> {
            if (isClicked || isMissed || paused) return;
            isClicked = true;
            timerToChomp.cancel();
            ...
            if (FlxG.save.data.genGlitch)
                ...
                FlxTween.num(1, 0, 0.75, {ease: FlxEase.circIn, onComplete: () -> {newHead.destroy(); remove(newHead, true);}}, (v) -> {newHead.alpha = v;});
        ...
        timerToChomp = new FlxTimer().start(time + (0.25 * i), (_) -> {
            ...
            newHead.animation.finishCallback = (name: String) -> {
                if (name == 'CHOMP') {
                    ...
                    for (cams in [camHUD,camNmi,camTails]){ //camInsideTV
                        cams.flash(FlxColor.RED, 1, null, true);
                        cams.shake(0.01, 0.25);
                    }
                    camHUD.flash(FlxColor.RED, 1, null, true);
                    camHUD.shake(0.01, 0.25);
                    cam.flash(FlxColor.RED, 0.2, null, true);
                    cam.shake(0.01, 0.50);
                    shatteredScreen.alpha = 1;
                    if (FlxG.save.data.moyDrainHealth) health -= 0.5;
                    healthTarget = health;
                    chomping.play(true);
...
function update(e) {
    if (crtFx != null) crtFx.iTime = e * 5;
    if (crtFxCamGAME != null) crtFxCamGAME.iTime = e * 5;
...
function postUpdate(e) {    
    ...
    if (skipOnce && FlxG.keys.justPressed.SPACE) {skip(); FlxTween.tween(skipTxt, { alpha: 0 }, 1, { ease: FlxEase.quadIn }); }
}
...
function redGuy() {
    ...
    doLerpStage = false;
    decreaseStageAl('-1');
    if (crtFxCamGAME != null) camGame.removeShader(crtFxCamGAME);
