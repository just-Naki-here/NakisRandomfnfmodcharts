...
function beatHit(_) {
    switch(_) {
        ...
        case 789:
            var user = (DiscordUtil.ready ? DiscordUtil.user.username : 'N4K2');

            add(rest = new FlxText(0, (FlxG.height - 60) - 55, FlxG.width, 'Go get some rest,  ' + user + '.')).setFormat(Paths.font('ArialCEMTBlack.ttf'), 26 + 5, 0xFFFEED34, 'center', FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
            rest.borderSize = 4;
            rest.screenCenter(FlxAxes.X);
            rest.cameras = [camTxt];

            new FlxTimer().start(0.1, () -> {
                if (user == 'Boyfriend') add(b = new FlxSprite(702, (FlxG.height - 60) - 53).makeSolid(195, rest.fieldHeight, FlxColor.BLACK)).cameras = [camTxt];
            });
