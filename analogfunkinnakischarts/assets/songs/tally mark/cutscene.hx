...
function create() {
    ...
    for (a in 0...2) {
        pages.push(new FunkinSprite(0, 0, Paths.image("cutscenes/tally mark-" + (a + 1))));
...
function update() {
    if (controls.ACCEPT && !end) {
        ...
        if (page == pages.length) {
            ...
            camCutscene.addShader(jpg);
            FlxTween.num(0.0000000001, 128, 1, null, _ -> jpg.pixel_size = _);
